...
@tool_metadata(
    ...
    annotations=ToolAnnotations(title="Get Plex logs", readOnlyHint=True),
)
async def server_get_plex_logs(
    ...
    log_type: Annotated[
        str,
        Field(
            description="Type of log to retrieve (server, scanner, transcoder, updater)",
            title="Log Type",
            examples=["server", "scanner", "transcoder", "updater"],
            default="server",
        ),
    ] = "server",
) -> str:
...
@tool_metadata(
    name="get_plex_server_info",
    description="Get detailed information about the Plex server",
    annotations=ToolAnnotations(title="Get Plex Server Info", readOnlyHint=True),
)
    ...
    try:
        ...
        server_info = {
            ...
            "my_plex_mapping_state": (
                plex.myPlexMappingState if hasattr(plex, "myPlexMappingState") else None
            ),
            ...
            "transcoder_audio": (
                plex.transcoderAudio if hasattr(plex, "transcoderAudio") else None
            ),
            ...
            "streaming_brain_version": (
                plex.streamingBrainVersion
                if hasattr(plex, "streamingBrainVersion")
                else None
            ),
            "owner_features": (
                plex.ownerFeatures if hasattr(plex, "ownerFeatures") else None
            ),
        }
...
@tool_metadata(
    name="get_plex_bandwidth",
    description="Get bandwidth statistics from the Plex server",
    annotations=ToolAnnotations(title="Get Plex Bandwidth", readOnlyHint=True),
)
async def server_get_bandwidth(
    timespan: Annotated[
        str | None,
        Field(
            description="Time span for bandwidth data (months, weeks, days, hours, seconds)",
            title="Timespan",
            examples=["months", "weeks", "days", "hours", "seconds"],
            default=None,
        ),
    ] = None,
    lan: Annotated[
        bool | None,
        Field(
            description="Filter by local network (true/false)",
            title="LAN Filter",
            examples=[True, False],
            default=None,
        ),
    ...
    try:
        ...
        if hasattr(plex, "bandwidth"):
            ...
            for bandwidth in [b for b in bandwidth_data if b is not None]:
                # Each bandwidth object has properties like accountID, at, bytes, deviceID, lan, timespan
                stats = {
                    "account": (
                        bandwidth.account().name
                        if bandwidth.account() and hasattr(bandwidth.account(), "name")
                        else None
                    ),
                    "device_id": (
                        bandwidth.deviceID if hasattr(bandwidth, "deviceID") else None
                    ),
                    "device_name": (
                    ...
                    "platform": (
                        bandwidth.device().platform
                        if bandwidth.device()
                        and hasattr(bandwidth.device(), "platform")
                        else None
                    ),
                    "client_identifier": (
                        bandwidth.device().clientIdentifier
                        if bandwidth.device()
                        and hasattr(bandwidth.device(), "clientIdentifier")
                        else None
                    ),
                    ...
                    "timespan (seconds)": (
                        bandwidth.timespan if hasattr(bandwidth, "timespan") else None
                    ),
                }
...
@tool_metadata(
    name="get_plex_server_resources",
    description="Get resource usage statistics from the Plex server",
    annotations=ToolAnnotations(title="Get Plex Server Resources", readOnlyHint=True),
)
async def server_get_current_resources() -> str:
    ...
    try:
        ...
        if hasattr(plex, "resources"):
            ...
            for resource in [r for r in server_resources if r is not None]:
...
@tool_metadata(
    name="get_plex_butler_tasks",
    description="Get information about Plex Butler tasks",
    annotations=ToolAnnotations(title="Get Plex Butler Tasks", readOnlyHint=True),
)
async def server_get_butler_tasks() -> str:
    ...
    try:
        ...
        verify = False if base_url.startswith("https") else True  # type: ignore
...
@tool_metadata(
    name="get_plex_server_alerts",
    description="Get real-time alerts from the Plex server",
    annotations=ToolAnnotations(title="Get Plex Server Alerts", readOnlyHint=True),
)
async def server_get_alerts(
    timeout: Annotated[
        int,
        Field(
            description="Number of seconds to listen for alerts",
            title="Timeout",
            examples=[5, 10, 15, 30],
            default=15,
        ),
    ...
    try:
        ...
        def alert_callback(data):
            ...
            try:
                ...
                alert_text = (
                    f"ALERT: {alert_type} - {alert_title} - {alert_description}"
                )
...
@tool_metadata(
    ...
    description="Run a specific Plex Butler task immediately",
    annotations=ToolAnnotations(title="Run Butler Task", readOnlyHint=False),
)
async def server_run_butler_task(
    task_name: Annotated[
        str,
        Field(
            description="Name of the butler task to run",
            title="Task Name",
            examples=[
                "BackupDatabase",
                "CheckForUpdates",
    ...
    try:
        ...
        if response.status_code in [200, 201, 202, 204]:
            return json.dumps(
                {
                    "status": "success",
                    "message": f"Butler task '{task_name}' started successfully",
                },
                indent=4,
            )
        else:
            ...
            error_message = (
                f"Failed to run butler task. Status code: {response.status_code}"
            )
