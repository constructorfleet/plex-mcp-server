...
@tool_metadata(
    name="get_active_plex_sessions",
    description="Get information about current playback sessions, including IP addresses.",
    annotations=ToolAnnotations(
        title="Get Active Plex Sessions",
        readOnlyHint=True,
        destructiveHint=False,
    ),
)
async def sessions_get_active() -> str:
    """
        ...
        for session in enumerate(sessions, 1):
            ...
            if hasattr(session, "viewOffset") and hasattr(session, "duration"):
                ...
                session_info["progress"] = {
                    ...
                    "minutes_remaining": (
                        int(minutes_remaining) if minutes_remaining > 1 else 0
                    ),
                }
            ...
            if transcode_session:
                ...
                if hasattr(transcode, "sourceVideoCodec") and hasattr(
                ...
                if hasattr(transcode, "sourceAudioCodec") and hasattr(
...
@tool_metadata(
    name="get_plex_media_playback_history",
    description="Get playback history for a specific media item by title or ID.",
    annotations=ToolAnnotations(
        title="Get Plex Media Playback History",
        readOnlyHint=True,
        destructiveHint=False,
        idempotentHint=True,
        openWorldHint=False,
...
async def sessions_get_media_playback_history(
    media_title: Annotated[
        str | None,
        Field(
            description="Title of the media to get history for. Optional if media_id is provided.",
            default=None,
            title="Media Title",
            examples=[
                "Breaking Bad",
    ...
    media_id: Annotated[
        int | None,
        Field(
            description="Plex media ID/rating key to directly fetch the item. Optional if media_title is provided.",
            default=None,
            title="Media ID",
            examples=[12345, 67890, 112233, 445566, 778899],
        ),
    ] = None,
) -> str:
    ...
    try:
        ...
        # Otherwise search by title
            ...
            if len(results) > 1:
                matches = []
                for item in [r for r in results if r is not None]:
                    item_info = {
                    ...
                    if item.type == "episode":
                        item_info["show_title"] = getattr(
                            item, "grandparentTitle", "Unknown Show"
                        )
                        item_info["season"] = getattr(
                            item, "parentTitle", "Unknown Season"
                        )
            ...
            return json.dumps(
                {"error": "Either media_id or media_title must be provided"},
                indent=4,
            )
        if not media:
            return json.dumps(
                {
                    "status": "error",
                    "message": f"No media found with title '{media_title}' or ID '{media_id}'.",
                },
                indent=4,
            )
