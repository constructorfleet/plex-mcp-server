...
@tool_metadata(
    ...
    description="Get statistics for a specific Plex library, including item counts and top genres/directors.",
    annotations=ToolAnnotations(
        title="Get Plex Library Statistics",
        readOnlyHint=True,
    ),
)
async def library_get_stats(
    library_name: Annotated[
        str,
        Field(
            description="Name of the Plex library to get stats for",
    ...
    try:
        ...
        async with httpx.AsyncClient() as session:
            ...
            sections_url = urljoin(base_url, "library/sections")  # type: ignore
            ...
            all_items_url = urljoin(base_url, f"library/sections/{section_id}/all")  # type: ignore
            unwatched_url = urljoin(
                base_url, f"library/sections/{section_id}/all?unwatched=1"  # type: ignore
            )
            ...
            if library_type == "movie":
                ...
                # Prepare URLs for concurrent requests
                seasons_url = urljoin(
                    base_url, f"library/sections/{section_id}/all?type=3"  # type: ignore
                )
                episodes_url = urljoin(
                    base_url, f"library/sections/{section_id}/all?type=4"  # type: ignore
                )
                ...
                for artist in all_data.get("Metadata", []):
                    ...
                    artist_tracks_url = urljoin(
                        ...
                        f"library/sections/{section_id}/all?artist.id={artist_id}&type=10",
                    )
                    artist_tracks_data = await async_get_json(
                        session, artist_tracks_url, headers
                    )
                    ...
                    if (
                        ...
                        for track in artist_tracks_data["MediaContainer"]["Metadata"]:
                            ...
                            if (
                                ...
                                audio_formats[audio_codec] = (
                                    audio_formats.get(audio_codec, 0) + 1
                                )
                ...
                if all_genres:
                    artist_stats["topGenres"] = dict(
                        sorted(all_genres.items(), key=lambda x: x[1], reverse=True)[
                            :10
                        ]
                    )
                if top_artists:
                    artist_stats["topArtists"] = dict(
                        sorted(top_artists.items(), key=lambda x: x[1], reverse=True)[
                            :10
                        ]
                    )
                if top_albums:
                    artist_stats["topAlbums"] = dict(
                        sorted(top_albums.items(), key=lambda x: x[1], reverse=True)[
                            :10
                        ]
                    )
                if all_years:
...
@tool_metadata(
    name="refresh_plex_library",
    description="Refresh a specific library or all libraries on the Plex server.",
    annotations=ToolAnnotations(
        title="Refresh Plex Library",
        readOnlyHint=False,
    ),
)
async def library_refresh(
    library_name: Annotated[
        str | None,
        Field(
            description="Name of the Plex library to refresh. If not provided, all Plex libraries will be refreshed.",
            default=None,
            examples=["Movies", "TV Shows"],
        ),
    ],
...
@tool_metadata(
    ...
    description="Scan a specific Plex library or part of a Plex library to update its contents.",
    annotations=ToolAnnotations(
...
async def library_scan(
    library_name: Annotated[
        str,
        Field(
            description="Name of the Plex library to scan",
            examples=["Movies", "TV Shows"],
        ),
    ],
    path: Annotated[
        str | None,
        Field(
            description="Optional specific path to scan within the Plex library. If not provided, the entire Plex library will be scanned.",
            default=None,
        ),
    ] = None,
) -> str:
...
@tool_metadata(
    name="get_plex_library_details",
    description="Get detailed information about a specific Plex library, including folder paths and settings.",
    annotations=ToolAnnotations(
        title="Get Plex Library Details",
        readOnlyHint=True,
    ),
)
async def library_get_details(
    library_name: Annotated[
        str,
        Field(
            description="Name of the Plex library to get details for",
            examples=["Movies", "TV Shows"],
        ),
    ],
) -> str:
...
@tool_metadata(
    name="get_plex_recently_added_media",
    description="Get recently added media across all Plex libraries or in a specific Plex library.",
    annotations=ToolAnnotations(
        title="Get Recently Added Plex Media",
        readOnlyHint=True,
    ),
)
async def library_get_recently_added(
    count: Annotated[
        ...
        Field(
            ...
            le=1000,
            default=50,
            examples=[10, 25, 50, 100, 250, 500, 1000],
        ),
    ] = 50,
    library_name: Annotated[
        str | None,
        Field(
            description="Optional name of the Plex library to limit results to. If not provided, returns recently added media from all Plex libraries.",
            default=None,
            examples=["Movies", "TV Shows"],
        ),
    ] = None,
    ...
    try:
        ...
        if library_name:
            ...
            if recent:
                try:
                    recent = sorted(
                        [r for r in recent if r is not None],
                        key=lambda x: getattr(x, "addedAt", -(1**10)),
                        reverse=True,
                    )[:count]
            ...
            try:
                ...
                if item_type == "movie":
                    ...
                    result["items"][item_type].append(
                        {
                            "showTitle": getattr(
                                item, "grandparentTitle", "Unknown Show"
                            ),
                    ...
                    result["items"][item_type].append(
                        {"title": item.title, "addedAt": added_at}
                    )
                    ...
                    result["items"][item_type].append(
                        {
                            "artist": getattr(
                                item, "grandparentTitle", "Unknown Artist"
                            ),
...
@tool_metadata(
    name="get_plex_library_contents",
    description="Get the full contents of a specific Plex library, including all items and their details.",
    annotations=ToolAnnotations(
        title="Get Plex Library Contents",
        readOnlyHint=True,
    ),
)
async def library_get_contents(
    library_name: Annotated[
        str,
        Field(
            description="Name of the Plex library to get contents from",
            examples=["Movies", "TV Shows"],
        ),
    ],
) -> str:
        ...
        async with httpx.AsyncClient() as session:
            ...
            if library_type == "movie":
                ...
                show_urls = [
                    (
                        item["ratingKey"],
                        urljoin(base_url, f'library/metadata/{item["ratingKey"]}'),  # type: ignore
                    )
                    for item in all_data.get("Metadata", [])
                ]
                ...
                for item, show_data in zip(
                    all_data.get("Metadata", []), show_responses
                ):
                    ...
                    watched = (
                        episode_count > 0
                        and show_data.get("viewedLeafCount", 0) == episode_count
                    )
                    ...
                    if (
                        ...
                        for track in artist_tracks_data["MediaContainer"]["Metadata"]:
                            ...
                            if "parentTitle" in track and track["parentTitle"]:
                                artists_info[artist_name]["albums"].add(
                                    track["parentTitle"]
                                )
