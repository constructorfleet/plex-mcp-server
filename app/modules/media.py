...
@tool_metadata(
    name="search_plex_media",
    description="Search for media across all libraries in Plex.",
    annotations=ToolAnnotations(
        title="Search Plex Media",
        readOnlyHint=True,
    ),
)
async def media_search(
    query: Annotated[
        ...
        Field(
            ...
            description="Search term to look for in Plex libraries",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
        ),
    ],
    content_type: Annotated[
        ...
        Field(
            title="Content Type",
            description="Optional content type to limit search to (movie, show, episode, track, album, artist or use comma-separated values for HTTP API like movies,music,tv)",
            examples=["movie", "show", "episode", "track", "album", "artist"],
            default=None,
        ),
    ] = None,
) -> str:
...
@tool_metadata(
    name="get_plex_media_details",
    description="Get detailed information about a specific media item in Plex.",
    annotations=ToolAnnotations(
        title="Get Plex Media Details",
        readOnlyHint=True,
    ),
)
async def media_get_details(
    media_title: Annotated[
        ...
        Field(
            title="Media Title",
            description="Title of the media to get details for. If media_id is provided, this can be omitted.",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
            default=None,
        ),
    ] = None,
    media_id: Annotated[
        ...
        Field(
            title="Media ID",
            description="Plex media ID/rating key to directly fetch the item. If media_title is provided, this can be omitted.",
    ...
    library_name: Annotated[
        ...
        Field(
            title="Library Name",
            description="Optional library name to limit search to when using media_title. If not provided, searches all libraries.",
    ...
    try:
        ...
        if media_id is not None:
            ...
            try:
                ...
                return json.dumps(
                    {
                        "error": f"Could not find media with ID {media_id}. Error: {str(e)}"
                    },
                    indent=4,
                )
        else:
            ...
            if library_name:
                ...
                if not results or any(
                    ...
                    for library in music_libraries:
                        ...
                        track_results = library.search(
                            query=media_title, libtype="track"
                        )
                        results.extend(track_results)

                        # Try searching for albums
                        album_results = library.search(
                            query=media_title, libtype="album"
                        )
                        results.extend(album_results)

                        # Try searching for artists
                        artist_results = library.search(
                            query=media_title, libtype="artist"
                        )
                        results.extend(artist_results)

            if not results:
                return json.dumps(
                    {"error": f"No media found matching '{media_title}'."}, indent=4
                )
            ...
            if len(results) > 1:
                ...
                simplified_results = [
                    item for item in simplified_results if item["id"] is not None
                ]
...
def get_media_details(media):
    ...
    details = {
        ...
        "added_at": (
            getattr(media, "addedAt").strftime("%Y-%m-%d %H:%M:%S")
        ...
        "duration": (
            format_duration(getattr(media, "duration", None))
    ...
    if media.type == "movie":
        ...
        try:
            ...
            details["seasons_count"] = len(media.seasons()) if hasattr(media, "seasons") and callable(media.seasons) else 0  # type: ignore
            details["episodes_count"] = len(media.episodes()) if hasattr(media, "episodes") and callable(media.episodes) else 0  # type: ignore
            ...
            if hasattr(media, "seasons") and callable(media.seasons):
                ...
                for season in seasons:  # type: ignore
                    season_data = {
                        "title": getattr(
                            season,
                            "title",
                            f"Season {getattr(season, 'index', 'Unknown')}",
                        ),
                    ...
                    if hasattr(season, "episodes") and callable(season.episodes):
                        try:
                            ...
                            season_data["episodes_count"] = len(episodes)  # type: ignore

                            for episode in episodes:  # type: ignore
                                episode_data = {
        ...
        try:
            ...
            details["albums_count"] = len(media.albums()) if hasattr(media, "albums") and callable(media.albums) else 0  # type: ignore
            details["tracks_count"] = len(media.tracks()) if hasattr(media, "tracks") and callable(media.tracks) else 0  # type: ignore
            details["rating"] = (
                getattr(media, "userRating", None)
                if hasattr(media, "userRating")
                else getattr(media, "rating", None)
                    ...
                    albums_list.append(
                        {
                            ...
                            "tracks_count": len(album.tracks()) if hasattr(album, "tracks") and callable(album.tracks) else 0,  # type: ignore
                        }
                    )
        ...
        try:
            ...
            if hasattr(media, "tracks") and callable(media.tracks):
                tracks = media.tracks()
                details["tracks_count"] = len(tracks)  # type: ignore
                    ...
                    tracks_list.append(
                        {
                            ...
                            "duration": (
                                format_duration(track_duration)
                                if track_duration
                                else None
                            ),
                        }
...
@tool_metadata(
    name="edit_plex_media_metadata",
    description="Edit metadata for a specific media item in Plex.",
    annotations=ToolAnnotations(
        title="Edit Plex Media Metadata",
        readOnlyHint=False,
...
async def media_edit_metadata(
    media_title: Annotated[
        ...
        Field(
            title="Media Title",
            description="Title of the media to edit. If multiple items match, specify library_name.",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
        ),
    ],
    library_name: Annotated[
        ...
        Field(
            title="Library Name",
            description="Optional library name to limit search to. If not provided, searches all libraries.",
            examples=["Movies", "TV Shows", "Music"],
            default=None,
        ),
    ] = None,
    new_title: Annotated[
        ...
        Field(
            title="New Title",
            description="New title for the media item. If not provided, keeps the current title.",
            examples=["Inception (2010)", "The Office (US)"],
            default=None,
        ),
    ] = None,
    new_summary: Annotated[
        ...
        Field(
            title="New Summary",
            description="New summary/description for the media item. If not provided, keeps the current summary.",
            examples=[
    ...
    new_rating: Annotated[
        float | None,
        Field(
            title="New Rating",
            description="New rating for the media item (0-10). If not provided, keeps the current rating.",
            examples=[8.8, 9.5],
            default=None,
        ),
    ] = None,
    new_release_date: Annotated[
        str | None,
        Field(
            title="New Release Date",
            description="New release date for the media item in YYYY-MM-DD format. If not provided, keeps the current release date.",
            examples=["2010-07-16", "2005-03-24"],
            default=None,
        ),
    ...
    new_genre: Annotated[
        ...
        Field(
            title="New Genre",
            description="New genre to add to the media item. If not provided, keeps the current genres.",
            examples=["Science Fiction", "Comedy", "Drama"],
            default=None,
        ),
    ] = None,
    remove_genre: Annotated[
        str | None,
        Field(
            title="Genre to Remove",
            description="Genre to remove from the media item. If not provided, keeps the current genres.",
            examples=["Action", "Adventure"],
            default=None,
        ),
    ] = None,
    new_director: Annotated[
        str | None,
        Field(
            title="New Director",
            description="New director to add to the media item (movies only). If not provided, keeps the current directors.",
            examples=["Christopher Nolan", "Greg Daniels"],
            default=None,
        ),
    ] = None,
    new_studio: Annotated[
        str | None,
        Field(
            title="New Studio",
            description="New studio to set for the media item. If not provided, keeps the current studio.",
            examples=["Warner Bros.", "Universal Pictures"],
            default=None,
        ),
    ] = None,
    new_tags: Annotated[
        ...
        Field(
            title="New Tags",
            description="List of tags to add to the media item. If not provided, keeps the current tags.",
            examples=[["thriller", "mind-bending", "dreams"], ["comedy", "sitcom"]],
            default=None,
        ),
    ] = None,
) -> str:
    ...
    try:
        ...
        if new_genre:
            try:
                if hasattr(media, "addGenre"):
                    ...
                    existing_genres = [
                        g.tag.lower() for g in getattr(media, "genres", [])
                    ]
        ...
        if remove_genre:
            try:
                if hasattr(media, "removeGenre"):
                    ...
                    matching_genres = [
                        g for g in media.genres if g.tag.lower() == remove_genre.lower()
                    ]
                    if matching_genres:
        ...
        if new_director and hasattr(media, "addDirector"):
            try:
                ...
                existing_directors = [
                    d.tag.lower() for d in getattr(media, "directors", [])
                ]
            ...
            for tag in new_tags:
                try:
                    if hasattr(media, "addLabel"):
                        ...
                        existing_labels = [
                            l.tag.lower() for l in getattr(media, "labels", [])
                        ]
...
@tool_metadata(
    name="get_plex_media_artwork",
    description="Get images for a specific media item in Plex.",
    annotations=ToolAnnotations(
        title="Get Plex Media Artwork",
        readOnlyHint=True,
    ),
)
async def media_get_artwork(
    media_title: Annotated[
        ...
        Field(
            title="Media Title",
            description="Title of the media to get images for. If media_id is provided, this can be omitted.",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
            default=None,
        ),
    ] = None,
    ...
    image_types: Annotated[
        ...
        Field(
            title="Image Types",
            description="List of image types to get. Options are: poster, art/background, logo, banner, thumb.",
            examples=[
                ["poster", "art", "logo"],
                ["thumb", "background"],
            ],
    ...
    output_format: Annotated[
        ...
        Field(
            title="Output Format",
            description="Format to return image data in. Options are: base64, url, or file_path.",
            examples=["base64", "url", "file_path"],
            default="base64",
        ),
    ] = "base64",
    output_dir: Annotated[
        ...
        Field(
            ...
            description="Directory to save images to when using file output format. Ignored for base64 and url formats.",
            examples=["/path/to/save/images", "./"],
            default="./",
        ),
    ] = "./",
) -> str:
    ...
    try:
        ...
        if media_id:
            try:
                ...
                if not media:
                    return json.dumps(
                        {"error": f"Media with ID '{media_id}' not found"}, indent=4
                    )
            except Exception as e:
                return json.dumps(
                    {"error": f"Error fetching media by ID: {str(e)}"}, indent=4
                )
        else:
        ...
        for img_type in image_types:
            ...
            if not hasattr(media, url_attr):
                result[img_type] = {
                    "error": f"This media item doesn't have {img_type} artwork"
                }
                continue
            ...
            if not img_url:
                result[img_type] = {
                    "error": f"No {img_type} artwork found for this media"
                }
                continue
            ...
            if (
                ...
                and hasattr(media, collection_method)
                and callable(getattr(media, collection_method))
            ):
                try:
            ...
            if response.status_code != 200:
                result[img_type] = {
                    "error": f"Failed to download {img_type} image: HTTP {response.status_code}"
                }
                continue
...
async def media_delete(
    media_title: Annotated[
        ...
        Field(
            title="Media Title",
            description="Title of the media to delete. If media_id is provided, this can be omitted.",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
            default=None,
        ),
    ] = None,
        ...
        if media_id:
            try:
                ...
                try:
                    ...
                    return json.dumps(
                        {"error": f"Error during deletion: {str(delete_error)}"},
                        indent=4,
                    )
            ...
            valid_media_results = []
            for item in results:
                if hasattr(item, "type") and getattr(item, "type", None) in [
                    "movie",
                    "show",
                    "episode",
                    "season",
...
@tool_metadata(
    name="set_plex_media_artwork",
    description="Set artwork for a specific media item in Plex.",
    annotations=ToolAnnotations(
        title="Set Plex Media Artwork",
        readOnlyHint=False,
        destructiveHint=True,
    ),
)
async def media_set_artwork(
    media_title: Annotated[
        ...
        Field(
            title="Media Title",
            description="Title of the media to set artwork for. If media_id is provided, this can be omitted.",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
        ),
    ],
    library_name: Annotated[
    ...
    art_type: Annotated[
        ...
        Field(
            title="Artwork Type",
            description="Type of artwork to set. Options are: poster, background/art, logo.",
            examples=["poster", "background", "art", "logo"],
            default="poster",
        ),
    ] = "poster",
    filepath: Annotated[
        ...
        Field(
            title="File Path",
            description="Path to the local image file to upload. If url is provided, this can be omitted.",
            examples=["/path/to/image.jpg", "./image.png"],
            default=None,
        ),
    ] = None,
    url: Annotated[
        ...
        Field(
            title="Image URL",
            description="URL to the image file to upload. If filepath is provided, this can be omitted.",
    ...
    lock: Annotated[
        ...
        Field(
            title="Lock Artwork",
            description="Whether to lock the artwork to prevent Plex from changing it. Default is False.",
            examples=[True, False],
            default=False,
        ),
    ] = False,
    ...
    try:
        ...
        if upload_method is None:
            return f"Invalid art type: {art_type}. Supported types: {', '.join(upload_map.keys())}"
        if not hasattr(media, upload_method):  # type: ignore
            return f"This media item doesn't support setting {art_type} artwork."

        # Upload the artwork
        upload_fn = getattr(media, upload_method)  # type: ignore
        ...
        if lock:
            ...
            if lock_method is not None and hasattr(media, lock_method):
...
@tool_metadata(
    name="list_plex_media_artwork",
    description="List all available artwork for a specific media item in Plex.",
    annotations=ToolAnnotations(
        title="List Plex Media Artwork",
        readOnlyHint=True,
    ),
)
async def media_list_available_artwork(
    media_title: Annotated[
        ...
        Field(
            title="Media Title",
            description="Title of the media to list artwork for. If media_id is provided, this can be omitted.",
            examples=["Inception", "The Office", "Bohemian Rhapsody", "Breaking Bad"],
            default=None,
        ),
    ] = None,
    ...
    art_type: Annotated[
        ...
        Field(
            title="Artwork Type",
            description="Type of artwork to list. Options are: poster, background/art, logo.",
            examples=["poster", "background", "art", "logo"],
            default="poster",
        ),
    ] = "poster",
    ...
    try:
        ...
        if not hasattr(media, art_method):
            return json.dumps(
                {"error": f"This media item doesn't support {art_type} artwork"},
                indent=4,
            )
        ...
        try:
            ...
            if not artwork_list:
                return json.dumps(
                    {"error": f"No {art_type} artwork found for media"}, indent=4
                )
            ...
            for i, art in enumerate(artwork_list, 1):
                art_data = {
                    ...
                    "rating_key": (
                        getattr(art, "ratingKey", None)
                        if hasattr(art, "ratingKey")
                        else None
                    ),
                }
            ...
            return json.dumps(
                {"error": f"Error retrieving {art_type} artwork: {str(art_error)}"},
                indent=4,
            )
    except Exception as e:
        return json.dumps(
            {"error": f"Error listing {art_type} artwork: {str(e)}"}, indent=4
        )
