...
from app.modules import connect_to_plex
...
@tool_metadata(
    name="search_plex_users",
    description="Search for Plex users by name, username, or email. If no search term is provided, list all users.",
    annotations=ToolAnnotations(title="Search Plex Users", readOnlyHint=True),
)
async def user_search_users(
    search_term: Annotated[
        ...
        Field(
            title="Search Term",
            description="Optional search term to filter users by name, username, or email. If not provided, lists all users.",
            examples=["john_doe"],
            default=None,
        ),
    ] = None,
            ...
            for user in found_users:
                ...
                if not is_owner and hasattr(user, "servers"):
                    ...
                    for server in user.servers:
                        if (
                            server.name == account.title
                            or server.name == account.username
                        ):
                    ...
                    result["sharedUsers"].append(
                        {
                            ...
                            "email": user.email if hasattr(user, "email") else None,
                            "title": (
                                user.title if hasattr(user, "title") else user.username
                            ),
                        }
                    )
...
@tool_metadata(
    name="get_plex_user_info",
    description="Get detailed information about a specific Plex user, including their role, email, and server access.",
    annotations=ToolAnnotations(title="Get Plex User Info", readOnlyHint=True),
)
async def user_get_info(
    username: Annotated[
        ...
        Field(
            title="Plex Username",
            description="Name of the user to get information for. Defaults to PLEX_USERNAME in .env if not provided.",
            examples=["john_doe"],
            default=PLEX_USERNAME,
        ),
    ] = PLEX_USERNAME,
) -> str:
    ...
    try:
        ...
        if not target_user:
            return json.dumps(
                {"error": f"User '{username}' not found among shared users."}
            )
        ...
        if hasattr(target_user, "devices") and callable(
            getattr(target_user, "devices")
        ):
            try:
...
@tool_metadata(
    name="get_plex_on_deck_media",
    description="Get on-deck (in progress) media for a specific user.",
    annotations=ToolAnnotations(title="Get Plex On Deck Media", readOnlyHint=True),
)
async def user_get_on_deck(
    username: Annotated[
        ...
        Field(
            title="Plex Username",
            description="Name of the user to get on-deck items for. Defaults to PLEX_USERNAME in .env if not provided.",
            examples=["john_doe"],
            default=PLEX_USERNAME,
        ),
    ] = PLEX_USERNAME,
    ...
    try:
        ...
        if (
            ...
            and username.lower() == plex.myPlexAccount().username.lower()
        ):
            ...
            try:
                ...
                target_user = None
                for user in [u for u in account.users() if u is not None]:
                    if (
                ...
                return json.dumps(
                    {"error": f"Error accessing user '{username}': {str(user_err)}"}
                )
        else:
            return json.dumps({"error": "Missing required field: username"})
        ...
        for item in on_deck_items:
            ...
            item_data = {"type": media_type, "title": title}
...
@tool_metadata(
    name="get_plex_watch_history",
    description="Get recent watch history for a specific user, with optional content type filtering.",
    annotations=ToolAnnotations(title="Get Plex Watch History", readOnlyHint=True),
)
async def user_get_watch_history(
    username: Annotated[
        ...
        Field(
            title="Plex Username",
            description="Name of the user to get watch history for. Defaults to PLEX_USERNAME in .env if not provided.",
            examples=["john_doe"],
            default=PLEX_USERNAME,
        ),
    ] = PLEX_USERNAME,
    limit: Annotated[
        ...
        Field(
            title="Limit",
            description="Maximum number of recently watched items to show. Defaults to 10.",
            default=10,
            examples=[1, 10, 6],
        ),
    ] = 10,
    content_type: Annotated[
        ...
        Field(
            title="Content Type",
            description="Optional filter for content type (movie, show, episode, etc). If not provided, shows all types.",
            examples=["movie", "show", "episode"],
            default=None,
        ),
    ] = None,
) -> str:
    ...
    try:
        ...
        max_attempts = (
            4  # Maximum number of search expansions to prevent infinite loops
        )
        attempt = 0
        ...
        while len(filtered_items) < limit and attempt < max_attempts:
            ...
            if username.lower() == account.username.lower():
                ...
                history_items = plex.history(
                    maxresults=current_search_limit, accountID=target_user.id
                )
...
@tool_metadata(
    name="get_plex_user_statistics",
    description="Get statistics about user watch activity over different time periods.",
    annotations=ToolAnnotations(title="Get Plex User Statistics", readOnlyHint=True),
)
async def user_get_statistics(
    time_period: Annotated[
        ...
        Field(
            title="Time Period",
            description="Time period for statistics - options: 'last_24_hours', 'last_7_days', 'last_30_days', 'last_90_days', 'last_year', 'all_time'. Defaults to 'last_24_hours'.",
            examples=[
                "last_24_hours",
                "last_7_days",
                "last_30_days",
    ...
    username: Annotated[
        ...
        Field(
            title="Plex Username",
            description="Optional. Filter statistics for a specific user. If not provided, returns statistics for all users.",
            examples=["john_doe"],
            default=None,
        ),
    ] = None,
    ...
    try:
        ...
        if response.status_code != 200:
            return json.dumps(
                {"error": f"Failed to fetch statistics: HTTP {response.status_code}"}
            )
        ...
        if username:
            ...
            if username.lower() == account.username.lower():
                ...
                for user in account.users():
                    if user.username.lower() == username.lower() or (
                        hasattr(user, "title")
                        and user.title.lower() == username.lower()
                    ):
            ...
            if target_account_id is None:
                return json.dumps(
                    {"error": f"User '{username}' not found in the statistics data."}
                )
        ...
        # Media type mapping
        media_type_map = {1: "movie", 4: "episode", 10: "track", 100: "photo"}
        ...
        for stat in stats_list:
            ...
            if account_id not in user_stats:
                account_info = account_lookup.get(
                    account_id, {"name": f"Unknown User {account_id}"}
                )
            ...
            if device_id is not None:
                device_info = device_lookup.get(
                    device_id,
                    {"name": f"Unknown Device {device_id}", "platform": "unknown"},
                )
        ...
        for account_id, stats in user_stats.items():
            ...
            for media_type, media_stats in stats["media_types"].items():
                ...
                minutes, seconds = divmod(remainder, 60)
                media_stats["formatted_duration"] = (
                    f"{int(hours)}h {int(minutes)}m {int(seconds)}s"
                )
            ...
            for device_name, device_stats in stats["devices"].items():
                ...
                minutes, seconds = divmod(remainder, 60)
                device_stats["formatted_duration"] = (
                    f"{int(hours)}h {int(minutes)}m {int(seconds)}s"
                )

        # Sort users by total duration (descending)
        sorted_users = sorted(
            user_stats.values(), key=lambda x: x["total_duration"], reverse=True
        )
